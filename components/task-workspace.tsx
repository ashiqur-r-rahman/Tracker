...
type Task = { id: string; task_what: string; goal: string | null; source: string | null; source_link: string | null; expected_duration_minutes: number | null; task_state: "todo" | "running" | "done"; completion_summary: string | null; users_profile?: { display_name: string; username: string }[] };
...
export function TaskWorkspace({ projectId, tasks }: { projectId: string; tasks: Task[] }) {
  ...
  return <section className="projects task-workspace"><div className="panel-title"><h2>Shared tasks</h2><button onClick={() => setAdding(!adding)}>{adding ? "Close" : "Add task"}</button></div>{adding && <TaskForm projectId={projectId} onDone={() => { setAdding(false); router.refresh(); }} />}{error && <p className="form-error">{error}</p>}<div className="task-columns">{groups.map((state) => <div className="task-column" key={state}><h3 className={`task-heading ${state}`}>{state}</h3>{tasks.filter((task) => task.task_state === state).map((task) => { const creator = task.users_profile?.[0]; const username = creator?.username || "member"; return <article className={`task-card user-${username}`} key={task.id}><button className="delete-icon" aria-label={`Delete ${task.task_what}`} title="Delete task" onClick={() => remove(task)}>x</button><b>{task.task_what}</b><small>Created by {username}</small>{task.goal && <p>{task.goal}</p>}<small>{task.source || "No source"}{task.expected_duration_minutes ? ` / ${task.expected_duration_minutes} min` : ""}</small>{task.completion_summary && <em>Summary: {task.completion_summary}</em>}{state !== "done" && <button onClick={() => state === "running" ? setSummaryTask(task) : advance(task)}>{state === "todo" ? "Start running" : "Mark done"}</button>}</article>; })}</div>)}</div>{summaryTask && <CompletionForm task={summaryTask} onSubmit={(summary) => advance(summaryTask, summary)} onCancel={() => setSummaryTask(null)} />}</section>;
}
